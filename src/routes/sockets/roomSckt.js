...
const RoomMongo = require('../../models/RoomMongo');

module.exports = roomSckt = (io, socket, currentPlayer) => {
  //checked
  socket.on('enter-room', async (req) => {
    const roomId = req.roomId;
    const found_room_mongo = await RoomMongo.findById(roomId);
    const found_room = new RoomMongo({
      _id: found_room_mongo._id,
      name: found_room_mongo.name,
      players: found_room_mongo.players,
      status: found_room_mongo.status,
      maxPlayers: found_room_mongo.maxPlayers,
    });
    const nickName = req.nickName;
    if (!nickName) socket.emit('room-response', new Response('Wrong nickname'));
    else if (found_room) {
      socket.join(found_room.name);
      roomPlayerCheck(found_room, nickName);
      startGameFlag(found_room);
      let room = await RoomMongo.findOneAndUpdate(
        {
          _id: roomId,
        },
        {
    ...
    } else socket.emit('room-response', new Response('Wrong room id'));
  });
  ...
  const startGameFlag = (room) => {
    if (room.isFull() && room.status !== 'Playing') {
      room.status = 'Playing';
      room.save(); //if some shit happens use fuckin update function
      games.push(gameFunctions.initializeGame(io, room));
    }
  };

  //checked
  const roomPlayerCheck = async (room, nickName) => {
    if (
      !room.isPlayerExist(currentPlayer.id) &&
      room.status !== 'Playing' &&
      nickName
    ) {
      await RoomMongo.updateOne(
        ...
        { $push: { players: [JSON.stringify(currentPlayer)] } }
      );
      ...
      socket.emit(
        'room-response',
        new Response(
          "You've successfully entered to the room",
          200,
          currentPlayer
        )
      );
  ...
  socket.on('make-turn', (card, relatedInfo) => {
    try {
      ...
      const moveResult = move(
        currentGame,
        card,
        currentGame.findPlayerById(currentPlayer.id),
        relatedInfo,
        cardResponse
      );
      let moveResponse;
      switch (moveResult) {
        case 'Success':
          ...
          if ((winner = gameFunctions.checkForWinner(currentGame))) {
            io.to(currentGame.room.name).emit(
              'win',
              new Response('Win', 200, winner)
            );
      ...
      switch (card.power) {
        case 2:
          io.to(currentPlayer.socketId).emit(
            'card-priest',
            new Response('card-priest', 200, {
              targetPlayerId: relatedInfo.targetPlayerId,
              cardResponseResult: cardResponse.result,
            })
          );
          break;
        case 5:
          io.to(currentPlayer.socketId).emit(
            'card-priest',
            new Response('card-prince', 200, relatedInfo.targetPlayerId)
          );
          break;
        case 6:
          io.to(currentPlayer.socketId).emit(
            'card-priest',
            new Response('card-king', 200, {
      ...
      gameFunctions.sendPlayersWithoutCards(
  ...
  socket.on('new-room', async (req) => {
    ...
    if (roomName && maxPlayers && isNumber(maxPlayers)) {
      let createdRoom = new RoomMongo();
      // /roomName, 'Waiting', maxPlayers
      createdRoom.name = roomName;
      createdRoom.status = 'Waiting';
      createdRoom.maxPlayers = maxPlayers;
      console.log(createdRoom);
      await createdRoom.save();
      // await rooms.write(createdRoom);
      socket.emit('created-room', new Response('Done', 200, createdRoom));
      await sendWaitingRooms();
    } else socket.emit('created-room', new Response('Wrong arguments'));
  });

  const sendWaitingRooms = async () => {
    // const allRooms = await RoomMongo.find();
    const waitingRooms = await RoomMongo.find({ status: 'Playing' });
